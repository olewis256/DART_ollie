...
!      case(NOAA_1_VTPR1_RADIANCE:CTT)
...
type visir_metadata_type
   ...
   integer(jpim) :: channel_1
   integer(jpim) :: channel_2
...
logical,                                intent(in)  :: cloud_on
logical,                                intent(in)  :: overcast_on
...
runtime % opts % rt_ir % addclouds = cloud_on
runtime % opts % rt_ir % overcast = overcast_on

write(*,*) "cloud: ", runtime % opts % rt_ir % addclouds
...
DO imem = 1, ens_size
   ...
   write(*,*) size(runtime % profiles(imem) % p(:))
   write(*,*) "take it back"
   write(*,*) size(atmos % pressure(imem,:)/100.0_jprb)
...
missing_visir_metadata%channel_1    = MISSING_I
missing_visir_metadata%channel_2    = MISSING_I
...
missing_mw_metadata%channel_1    = MISSING_I
missing_mw_metadata%channel_2    = MISSING_I
...
subroutine set_visir_metadata(key, sat_az, sat_ze, sun_az, sun_ze, &
   platform_id, sat_id, sensor_id, channel_1, channel_2, specularity)

integer,  intent(out) :: key
real(r8), intent(in)  :: sat_az, sat_ze
real(r8), intent(in)  :: sun_az, sun_ze ! only relevant if addsolar
integer,  intent(in)  :: platform_id, sat_id, sensor_id, channel_1, channel_2
...
visir_obs_metadata(visirnum) % channel_1   = channel_1
visir_obs_metadata(visirnum) % channel_2   = channel_2
...
subroutine set_mw_metadata(key, sat_az, sat_ze, platform_id, sat_id, sensor_id, &
   channel_1, channel_2, mag_field, cosbk, fastem_p1, fastem_p2, fastem_p3,         &
   fastem_p4, fastem_p5)
...
mw_obs_metadata(mwnum) % channel_1    = channel_1
mw_obs_metadata(mwnum) % channel_2    = channel_2
...
integer,  intent(out) :: platform_id, sat_id, sensor_id, channel_1, channel_2
...
channel_1   = visir_obs_metadata(mykey) % channel_1
channel_2   = visir_obs_metadata(mykey) % channel_2
...
subroutine get_mw_metadata(key, sat_az, sat_ze,        &
   platform_id, sat_id, sensor_id, channel_1, channel_2, mag_field, &
...
channel_1    = mw_obs_metadata(mykey) % channel_1
channel_2    = mw_obs_metadata(mykey) % channel_2
...
integer           :: platform_id, sat_id, sensor_id, channel_1, channel_2
real(r8)          :: specularity
...
if ( is_asciifile ) then
   ...
   if (trim(header) == trim(VISIR_STRING)) then
      ...
      read(ifile, *, iostat=ierr) platform_id, sat_id, sensor_id, channel_1, channel_2
      call check_iostat(ierr,'read_rttov_metadata','platform/sat_id/sensor/channel_1',string2)
...
select case (obstype_metadata(SUBTYPE,key))
   ...
   case (VISIR)
      ...
      if (is_asciifile) then
         ...
         write(ifile, *) platform_id, sat_id, sensor_id, channel_1, channel_2
   ...
   case (MW)
      call get_mw_metadata(key, sat_az, sat_ze,                  &
         platform_id, sat_id, sensor_id, channel_1, channel_2, mag_field, cosbk, &
...
channel_1   = interactive_i('channel_1     Instrument channel number [see docs]', minvalue = 1)
channel_2   = interactive_i('channel_2     Instrument channel number [see docs]', minvalue = 1)
...
!----------------------------------------------------------------------
subroutine get_expected_radiance(obs_kind_ind, state_handle, ens_size, location, key, flavor, ctt, istatus)
...
real(r8),            intent(out) :: ctt(ens_size)     ! value of obs
integer,             intent(out) :: istatus(ens_size) ! status of the calculation

real(r8) :: radiance1(ens_size), radiance2(ens_size)
real(r8) :: radiance_clear1(ens_size), radiance_clear2(ens_size)
real(r8) :: radiance_opaque1(ens_size), radiance_opaque2(ens_size)

real(r8) :: f1(ens_size), f2(ens_size)

real(r8), allocatable :: cost(:,:)

integer  :: platform_id, sat_id, sensor_id, channel_1, channel_2
integer  :: instrument(3)

type(atmos_profile_type) :: atmos_search
...
ctt = 0.0_r8 ! set return value early
...
case (VISIR)
   ...
   channel_1   = visir_md % channel_1
   channel_2   = visir_md % channel_2
case (MW)
   visir_md => null()
   ...
   channel_1   = mw_md % channel_1
   channel_2   = mw_md % channel_2
case default
   call error_handler(E_ERR, routine, 'unknown metadata type', source, revision, revdate)
end select
...
GETLEVELDATA : do i = 1,numlevels
   ...
   call check_status('QTY_PRESSURE', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .true., return_now)
   if (return_now) return

   call interpolate(state_handle, ens_size, loc, QTY_TEMPERATURE, atmos%temperature(:, i), this_istatus)
   call check_status('QTY_TEMPERATURE', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .true., return_now)
   if (return_now) return

   call interpolate(state_handle, ens_size, loc, QTY_VAPOR_MIXING_RATIO, atmos%moisture(:, i), this_istatus)
   call check_status('QTY_VAPOR_MIXING_RATIO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .true., return_now)
   if (return_now) return

   if (ozone_data) then
      call interpolate(state_handle, ens_size, loc, QTY_O3, trace_gas%ozone(:, i), this_istatus)
      call check_status('QTY_O3', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if

   if (co2_data) then
      call interpolate(state_handle, ens_size, loc, QTY_CO2, trace_gas%co2(:, i), this_istatus)
      call check_status('QTY_CO2', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if

   if (n2o_data) then
      call interpolate(state_handle, ens_size, loc, QTY_N2O, trace_gas%n2o(:, i), this_istatus)
      call check_status('QTY_N2O', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if

   if (ch4_data) then
      call interpolate(state_handle, ens_size, loc, QTY_CH4, trace_gas%ch4(:, i), this_istatus)
      call check_status('QTY_CH4', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if

   if (co_data) then
      call interpolate(state_handle, ens_size, loc, QTY_CO, trace_gas%co(:, i), this_istatus)
      call check_status('QTY_CO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if

   if (so2_data) then
      call interpolate(state_handle, ens_size, loc, QTY_SO2, trace_gas%so2(:, i), this_istatus)
      call check_status('QTY_SO2', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (add_aerosl) then
      if (aerosl_type == 1) then
         ...
         call check_status('QTY_INSOLUBLE_AER', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_H2O_SOLUBLE_AER, aerosols%water_soluble(:, i), this_istatus)
         call check_status('QTY_H2O_SOLUBLE_AER', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_SOOT, aerosols%soot(:, i), this_istatus)
         call check_status('QTY_SOOT', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_SEASALT_ACCUM, aerosols%sea_salt_accum(:, i), this_istatus)
         call check_status('QTY_SEASALT_ACCUM', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_SEASALT_COARSE, aerosols%sea_salt_coarse(:, i), this_istatus)
         call check_status('QTY_SEASALT_COARSE', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_MINERAL_NUCLEUS, aerosols%mineral_nucleus(:, i), this_istatus)
         call check_status('QTY_MINERAL_NUCLEUS', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_MINERAL_ACCUM, aerosols%mineral_accum(:, i), this_istatus)
         call check_status('QTY_MINERAL_ACCUM', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_MINERAL_COARSE, aerosols%mineral_coarse(:, i), this_istatus)
         call check_status('QTY_MINERAL_COARSE', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_MINERAL_TRANSPORTED, aerosols%mineral_transport(:, i), this_istatus)
         call check_status('QTY_MINERAL_TRANSPORT', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_SULPHATED_DROPS, aerosols%sulphated_droplets(:, i), this_istatus)
         call check_status('QTY_SULPHATED_DROPS', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_VOLCANIC_ASH, aerosols%volcanic_ash(:, i), this_istatus)
         call check_status('QTY_VOLCANIC_ASH', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_NEW_VOLCANIC_ASH, aerosols%new_volcanic_ash(:, i), this_istatus)
         call check_status('QTY_NEW_VOLCANIC_ASH', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_ASIAN_DUST, aerosols%asian_dust(:, i), this_istatus)
         call check_status('QTY_ASIAN_DUST', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return
      elseif (aerosl_type == 2) then
         ! CAMS
         call interpolate(state_handle, ens_size, loc, QTY_BLACK_CARBON, aerosols%black_carbon(:, i), this_istatus)
         call check_status('QTY_BLACK_CARBON', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_DUST_BIN1, aerosols%dust_bin1(:, i), this_istatus)
         call check_status('QTY_DUST_BIN1', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_DUST_BIN2, aerosols%dust_bin2(:, i), this_istatus)
         call check_status('QTY_DUST_BIN2', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_DUST_BIN3, aerosols%dust_bin3(:, i), this_istatus)
         call check_status('QTY_DUST_BIN3', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_AMMONIUM_SULPHATE, aerosols%ammonium_sulphate(:, i), this_istatus)
         call check_status('QTY_AMMONIUM_SULPHATE', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_SEA_SALT_BIN1, aerosols%sea_salt_bin1(:, i), this_istatus)
         call check_status('QTY_SEA_SALT_BIN1', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_SEA_SALT_BIN2, aerosols%sea_salt_bin2(:, i), this_istatus)
         call check_status('QTY_SEA_SALT_BIN2', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return

         call interpolate(state_handle, ens_size, loc, QTY_SEA_SALT_BIN3, aerosols%sea_salt_bin3(:, i), this_istatus)
         call check_status('QTY_SEA_SALT_BIN3', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return
         ...
         call check_status('QTY_HYDROPHILIC_ORGANIC_MATTER', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return
      end if
   end if
   ...
   if (w_data) then
      ...
      call check_status('QTY_VERTICAL_VELOCITY', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (cfrac_data) then
      ...
      call check_status('QTY_CLOUD_FRACTION', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (clw_scheme == 2) then
      ...
      call check_status('QTY_CLOUDWATER_DE', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (clw_data) then
      ...
      call check_status('QTY_CLOUDWATER_MIXING_RATIO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (rain_data) then
      ...
      call check_status('QTY_RAINWATER_MIXING_RATIO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (ciw_data) then
      ...
      call check_status('QTY_ICE_MIXING_RATIO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
      ...
      if (ice_scheme == 1 .and. use_icede) then
         ...
         call check_status('QTY_CLOUD_ICE_DE', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
         if (return_now) return
      end if
   end if
   ...
   if (snow_data) then
      ...
      call check_status('QTY_SNOW_MIXING_RATIO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (graupel_data) then
      ...
      call check_status('QTY_GRAUPEL_MIXING_RATIO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
   ...
   if (hail_data) then
      ...
      call check_status('QTY_HAIL_MIXING_RATIO', ens_size, this_istatus, ctt, loc, istatus, routine, source, revision, revdate, .false., return_now)
      if (return_now) return
   end if
...
call check_status('QTY_SURFACE_ELEVATION', ens_size, this_istatus, ctt, loc_sea, istatus, routine, source, revision, revdate, .true., return_now)
if (return_now) return
...
call check_status('QTY_SURFACE_PRESSURE', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .true., return_now)
if (return_now) return

call interpolate(state_handle, ens_size, loc_2m, QTY_2M_TEMPERATURE, atmos%s2m_t(:), this_istatus)
call check_status('QTY_2M_TEMPERATURE', ens_size, this_istatus, ctt, loc_2m, istatus, routine, source, revision, revdate, .true., return_now)
if (return_now) return

call interpolate(state_handle, ens_size, loc_sfc, QTY_SKIN_TEMPERATURE, atmos%skin_temp(:), this_istatus)
call check_status('QTY_SKIN_TEMPERATURE', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .true., return_now)
if (return_now) return

! set to 2m if an error

call interpolate(state_handle, ens_size, loc_sfc, QTY_SURFACE_TYPE, atmos%surftype(:), this_istatus)
call check_status('QTY_SURFACE_TYPE', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .true., return_now)
if (return_now) return
...
if (use_q2m) then
   ...
   call check_status('QTY_2M_SPECIFIC_HUMIDITY', ens_size, this_istatus, ctt, loc_2m, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if

if (use_uv10m) then
   call interpolate(state_handle, ens_size, loc_10m, QTY_10M_U_WIND_COMPONENT, atmos%s10m_u(:), this_istatus)
   call check_status('QTY_10M_U_WIND_COMPONENT', ens_size, this_istatus, ctt, loc_10m, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
   call interpolate(state_handle, ens_size, loc_10m, QTY_10M_V_WIND_COMPONENT, atmos%s10m_v(:), this_istatus)
   call check_status('QTY_10M_V_WIND_COMPONENT', ens_size, this_istatus, ctt, loc_10m, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if

if (use_wfetch) then
   call interpolate(state_handle, ens_size, loc_sfc, QTY_WIND_FETCH, atmos%wfetch(:), this_istatus)
   call check_status('QTY_WIND_FETCH', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if

if (use_water_type) then
   call interpolate(state_handle, ens_size, loc_sfc, QTY_WATER_TYPE, atmos%water_type(:), this_istatus)
   call check_status('QTY_WATER_TYPE', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if

if (use_salinity) then
   call interpolate(state_handle, ens_size, loc_sfc, QTY_SALINITY, atmos%sfc_salinity(:), this_istatus)
   call check_status('QTY_SALINITY', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if

if (supply_foam_fraction) then
   call interpolate(state_handle, ens_size, loc_sfc, QTY_FOAM_FRAC, atmos%sfc_foam_frac(:), this_istatus)
   call check_status('QTY_FOAM_FRAC', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if

if (use_sfc_snow_frac) then
   call interpolate(state_handle, ens_size, loc_sfc, QTY_SNOWCOVER_FRAC, atmos%sfc_snow_frac(:), this_istatus)
   call check_status('QTY_SNOWCOVER_FRAC', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if
...
if (add_clouds .and. htfrtc_simple_cloud) then
   ...
   call check_status('QTY_COLUMN_CLOUD_FRAC', ens_size, this_istatus, ctt, loc_undef, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return

   call interpolate(state_handle, ens_size, loc_undef, QTY_CLOUD_TOP_PRESSURE, clouds%ctp(:), this_istatus)
   call check_status('QTY_CLOUD_TOP_PRESSURE', ens_size, this_istatus, ctt, loc_undef, istatus, routine, source, revision, revdate, .false., return_now)
   if (return_now) return
end if
...
if (debug) then
   ...
   print*, 'radiance = ', radiance1
   print*, 'radiance clear = ', radiance_clear1
   print*, 'radiance = ', radiance2
   print*, 'radiance clear = ', radiance_clear2
end if

f1 = (radiance1 - radiance_clear1) / (radiance2 - radiance_clear2)

allocate(cost(ens_size,numlevels))

GETPRESDATA : do i = 1, numlevels

   call atmos_profile_setup(atmos_search, ens_size, numlevels + 1 - i, use_q2m, & 
      use_uv10m, use_wfetch, use_water_type, use_salinity,       &
      supply_foam_fraction, use_sfc_snow_frac)

   call trace_gas_profile_setup(trace_gas, ens_size, numlevels + 1 - i,  &
      ozone_data, co2_data, n2o_data, ch4_data, co_data, so2_data)

   call cloud_profile_setup(clouds, ens_size, numlevels + 1 - i,     &
      cfrac_data, clw_data, clw_scheme, rain_data, ciw_data, &
      ice_scheme, use_icede, snow_data, graupel_data,        &
      hail_data, w_data, htfrtc_simple_cloud)
   ...
   call initialize_rttov_sensor_runtime(sensor,ens_size,numlevels + 1 - i)
   ...
   atmos_search%pressure(:,:) = atmos%pressure(:,i:numlevels)
   atmos_search%temperature(:,:) = atmos%temperature(:,i:numlevels)
   atmos_search%moisture(:,:) = atmos%moisture(:,i:numlevels)


   call interpolate(state_handle, ens_size, loc_sfc, QTY_SURFACE_PRESSURE, atmos_search%sfc_p(:), this_istatus)
   call check_status('QTY_SURFACE_PRESSURE', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .true., return_now)
   if (return_now) return

   call interpolate(state_handle, ens_size, loc_2m, QTY_2M_TEMPERATURE, atmos_search%s2m_t(:), this_istatus)
   call check_status('QTY_2M_TEMPERATURE', ens_size, this_istatus, ctt, loc_2m, istatus, routine, source, revision, revdate, .true., return_now)
   if (return_now) return

   call interpolate(state_handle, ens_size, loc_sfc, QTY_SKIN_TEMPERATURE, atmos_search%skin_temp(:), this_istatus)
   call check_status('QTY_SKIN_TEMPERATURE', ens_size, this_istatus, ctt, loc_sfc, istatus, routine, source, revision, revdate, .true., return_now)
   if (return_now) return


   call do_forward_model(ens_size=ens_size,                 &
                      nlevels=numlevels + 1 - i,            &
                      flavor=flavor,                        &
                      location=location,                    &
                      atmos=atmos_search,                   &
                      trace_gas=trace_gas,                  &
   ...
   f2 = (radiance_opaque1 - radiance_clear1) / (radiance_opaque2 - radiance_clear2)

   cost(:,i) = abs(f1 - f2)

   write(*,*) cost(:, i), atmos%   pressure(:,i)

   deallocate(atmos_search%   temperature,              &
              atmos_search%   moisture,                 &
              atmos_search%   pressure,                 &
              atmos_search%   sfc_p,                    &
              atmos_search%   s2m_t,                    &
              atmos_search%   skin_temp,                &
              atmos_search%   sfc_elev,                 &
              atmos_search%   surftype)
...
subroutine check_status(field_name, ens_size, this_istatus, ctt, loc, istatus, &
...
function get_channel(flavor, key) result(channel_1)
   ...
   channel_1 = MISSING_R8
   channel_2 = MISSING_R8
   ...
   case ( VISIR )
      call get_visir_metadata(key, &
                           ...
                           platform_id, sat_id, sensor_id, channel_1, channel_2, &
                           specularity)
   case ( MW )
   ...
   if (debug) write(*,*)'get_channel: key,channel ',key,channel_1
